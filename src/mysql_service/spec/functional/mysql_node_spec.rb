  ...
  it "should enforce max connection limitation per user account" do
    EM.run do
      ...
      EM.add_timer(1) do
        ...
        binding = node.bind(db["service_id"],  @default_opts, {'user' => 'user1', 'password' => 'abc'})
        @test_dbs[db] = [binding]
        expect { conn = connect_to_mysql(binding, 'abc') }.to_not raise_error
        EM.stop
      end
    end
  end
