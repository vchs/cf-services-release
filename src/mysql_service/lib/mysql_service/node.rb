...
class VCAP::Services::Mysql::Node
  ...
  def provision(plan, credential, version=nil, properties={})
    ...
    begin
      ...
      provisioned_service.run do |instance|
        setup_pool(instance)
        service_id, name, user = [:service_id, :name, :user].map { |field| instance.send(field) }
        result = is_restoring ? create_or_update_database_user(service_id, name, user, password) : create_database(instance, password)
  ...
  #TO-BE-FIXED in future: credential should contain password
