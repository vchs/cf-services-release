    ...
    def fetch_handles
      ...
      Common.config.services.each do |service_name|
        @fetch_handle_timers[service_name] = EM.add_periodic_timer(Common.config.handle_fetch_interval) do
          fetch_handles_from_gw("#{service_name}.#{CHAN_GW_HANDLES}")
        end
        EM.next_tick { fetch_handles_from_gw("#{service_name}.#{CHAN_GW_HANDLES}") }
      end
    end

    def fetch_handles_from_gw(channel)
      ...
      @message_bus.request(channel) do |resp|
        ...
        update_handles(message.service_name, message.handles, message.available)
      end
    end

    def update_handles(service_name, handles = {}, available = false)
      return if !available || handles.nil?
      logger.info("fetched #{handles.size} handles from #{service_name}")
      timer = @fetch_handle_timers[service_name]
