...
module BackupManager
  class Manager
    ...
    def initialize(options={})
      ...
      @gateway_timeout = Common.config.gateway_timeout || 10
    ...
    def fetch_handles_from_gw(service_name, channel)
      ...
      subscription = nil
      timer = EM.add_timer(@gateway_timeout) do
        @mesage_bus.unsubscribe(subscription)
        logger.error("time out when fetching handles from #{service_name}")
      end
      subscription = @message_bus.request(channel) do |resp|
    ...
    def update_handles(service_name, handles = {})
      ...
      unless @instance_handles.has_key? service_name
        ...
        EM.next_tick { setup_schedules(handles) }
      end
    end
